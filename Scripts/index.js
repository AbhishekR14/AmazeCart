...
document.querySelectorAll(".js-add-to-cart-button").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    const quantitySelector = document.querySelector(
      `.no-items-picked-button-${productId}`
    );
    const quantity = Number(quantitySelector.value);
    //adding item to cart
    addItemToCart(productId, quantity);
    document.querySelector(".js-cart-value").innerHTML =
      currentItemListQuantity();
    document.querySelector(".js-cart-text-dropdown").innerHTML =
      "Cart (" + currentItemListQuantity() + ")";
    //to make the "Added to cart " visible
    document
      .querySelector(`.js-added-to-cart-${productId}`)
      .classList.replace("added-to-cart", "added-to-cart-is-visible");
    setTimeout(function () {
      document
        .querySelector(`.js-added-to-cart-${productId}`)
        .classList.replace("added-to-cart-is-visible", "added-to-cart");
    }, 1500);
  });
});
...
document
  .querySelector(".js-search-bar-button")
    ...
    productsHTML += `
            <div class="element">
                ...
                <div class="stars-rating">
                    <img style="width: 100px;" src="Pictures/Ratings/rating-${
                ...
                <div class="no-items-picked">
                    <select class="no-items-picked-button no-items-picked-button-${
                      product.id
                    }">
...
function renderSearchedProducts(searchedText) {
  if (searchedText === "") {
    ...
    products.forEach((product) => {
      ...
      let Keywords = product.keywords.find(
        (element) => element.toLowerCase() === searchedText.toLowerCase()
      );
      if (
        name.includes(searchedText.toLowerCase()) ||
        Keywords === searchedText.toLowerCase()
          ? true
          : false
      ) {
        productsHTML += `
